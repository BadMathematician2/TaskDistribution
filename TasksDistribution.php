...
class TasksDistribution
{
    ...
    public function __construct($tasks, $instants, $times, $max_time)
    {
        ...
        foreach ($this->tasks as $key => $task) {
            $this->tasks_n_times[] = [$task => $this->times[$key]];
        }
        $this->time_sum = array_sum($this->times);
        $this->time = array_sum($this->times)/count($this->instants);
        $this->result = array_fill(0,count($this->instants),[]);
    }
    ...
    {
        $T = ceil($this->time);
        $b = true;  //показує чи є завдання із часом виконання, меншим ніж count($this->instants)
        $t = array_fill(0,count($this->instants),0); //масив, елементами якого є час виконання завдань відповідного виконавця завдань

        for ($i = 0; $i < count($this->instants); $i++)    //проходження по всім виконавцям завдань
        {
            while ($t[$i] < $T && count($this->times)>0 && $b)  //перша умова - це поки час усіх завдань менший за середній час, друга - поки є завдання, третя - див комент вище
            {
    ...
    {
        $sum = array_fill(0,count($this->instants),0);
        for ($i = 0; $i < count($this->instants); $i++)
        {
